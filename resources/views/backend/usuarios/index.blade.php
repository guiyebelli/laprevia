...
<div class="container">
	<div class="row transparente_BG blanco">
		...
		<div class="col-xs-12 text-center">
			<h3><span>USUARIOS DEL SISTEMA</span></h3>
			<a title="Nuevo usuario" href="{{ action('UsuariosController@create') }}" class="btn btn-rojo pull-right"> Nuevo</a>
		</div>
					...
					@if(count($usuarios) > 0)
						@foreach($usuarios as $usuario)
							<tr>
								...
								<td>{{$usuario->email}}</td>
								<td>
									<a title="Restablecer contraseña" data-toggle="tooltip" data-placement="bottom" href="{{ action('UsuariosController@restablecer_contra', [$usuario->id]) }}" data-method="patch" data-title="Restablecer contraseña" data-confirm="¿Esta seguro que desea restablecer la contraseña al usuario '{{$usuario->username}}'?"><span class="glyphicon glyphicon-refresh blanco"></span></a>
									  <!-- dejar esta línea q es un espacio entre los dos iconos -->
									<a title="Editar perfil" data-toggle="tooltip" data-placement="bottom" href="{{ action('UsuariosController@edit', [$usuario->id]) }}"><span class="glyphicon glyphicon-pencil blanco"></span></a>
									  <!-- dejar esta línea q es un espacio entre los dos iconos -->
									<a title="Eliminar" href="{{ action('UsuariosController@destroy', [$usuario->id]) }}" data-toggle="tooltip" data-placement="bottom" data-method="delete" data-confirm="¿Estas seguro que desea eliminar al usuario '{{$usuario->username}}'?"> <span class="glyphicon glyphicon-trash blanco"></span> </a>
								</td>
							</tr>
						@endforeach
					@else
